...
let bookingPopup = document.getElementById('popup-booking');
if (bookingPopup) {
	popupCalculator();
}
// Калькулятор
function popupCalculator() {
	// Смена экскурсий в зависимости от города

	var cities = document.querySelector('#cities');
	var excursions = document.querySelector('#excurtions');
	var options2 = excursions.querySelectorAll('option');

	function giveSelection(selValue) {
		excursions.innerHTML = '';
		for (var i = 0; i < options2.length; i++) {
			if (options2[i].dataset.option === selValue) {
				excursions.appendChild(options2[i]);
			}
		}
	}
	giveSelection(cities.value);

	// Сам расчет
	let sum = document.querySelector('.popup_booking__total');
	let adultTicket = document.querySelector('#adult');
	let childTicket = document.querySelector('#child');
	let childTicketFree = document.querySelector('#childfree');

	let excursionCost = 0;

	document.addEventListener('DOMContentLoaded', function () {
		let citiesBlock = document.querySelector('.popup_booking__select');
		let cityOptions = citiesBlock.querySelectorAll('.select__option');
		for (let index = 0; index < cityOptions.length; index++) {
			const cityOption = cityOptions[index];
			cityOption.addEventListener('click', function () {
				giveSelection(cities.value);
				inputsReset();
				adultTicket.innerHTML = 0 + '$';
				childTicket.innerHTML = 0 + '$';
				sum.innerHTML = 0 + '$';
			});
		}
	});
	...
	function excursionsSelect() {
		document.querySelector('.popup_booking__select-excursion').addEventListener('change', function () {
			excursionCost = document.getElementById('excurtions').value;
			sum.innerHTML = excursionCost + '$';
			adultTicket.innerHTML = excursionCost + '$';
			childTicket.innerHTML = 0 + '$';

			let extras = document.querySelectorAll('.popup_booking__extra');
			for (let index = 0; index < extras.length; index++) {
				const extra = extras[index];
				if (extra.id == 'adult-costs') {
					extra.dataset.extra = excursionCost + '$';
				} else if (extra.id == 'child-costs') {
					extra.dataset.extra = excursionCost / 2 + '$';
				} else {
					extra.dataset.extra == 0;
				}
			}
			// Сброс инпутов
			inputsReset();
	...
	function inputsReset() {
		let adultInput = document.getElementById('adult-input');
		adultInput.value = 1;
		let childInput = document.getElementById('child-input');
		childInput.value = 0;
		let childFreeInput = document.getElementById('childfree-input');
		childFreeInput.value = 0;
	}
		...
		if (quantityButtons.length > 0) {
			for (let index = 0; index < quantityButtons.length; index++) {
				...
				quantityButton.addEventListener('click', function () {
					if (excursionCost <= 0) {
						alert('Выберите экскурсию!');
					}
					if (excursionCost > 0) {
						let value = parseInt(quantityButton.closest('.quantity').querySelector('input').value);
						let subTotals = quantityButton.closest('.quantity').querySelectorAll('.popup_booking__subtotal');
						for (let index = 0; index < subTotals.length; index++) {
							const subTotal = subTotals[index];
							if (quantityButton.classList.contains('quantity__button_plus')) {
								value++;
								if (subTotal.id == 'child') {
									subTotal.innerHTML = (excursionCost * value) / 2 + '$';
								} else if (subTotal.id == 'childfree') {
									subTotal.innerHTML == 0;
								} else {
									subTotal.innerHTML = excursionCost * value + '$';
								}
							} else {
								value = value - 1;
								if (subTotal.id == 'child') {
					...
					let total = parseFloat(adultTicket.innerHTML) + parseFloat(childTicket.innerHTML);
					sum.innerHTML = total + '$';
				});
			}
		}
...
let bookingForm = document.getElementById('form-booking');
if (bookingForm) {
	formCalculator();
}
function formCalculator() {
	...
	let excursionCost = parseInt(document.querySelector('.popup_booking__subtitle span').innerHTML);
	console.log(excursionCost);
...
document.addEventListener('DOMContentLoaded', function () {
	...
	if (map) {
		setTimeout(function () {
			...
			elem.src =
				'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3204.4469855386196!2d30.574753815696305!3d36.56744497999726!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x14c3b6f9b9efdecf%3A0xa13c2682aad17e0a!2sLimak%20Limra%20Hotel%20%26%20Resort!5e0!3m2!1sru!2skz!4v1615549985333!5m2!1sru!2skz';
...
function swiperMode() {
	...
	if (tablet.matches) {
		if (!init) {
			init = true;
			cardsSlider = new Swiper('.services__slider', {
				observer: true,
				observeParents: true,
				slidesPerView: 4,
				spaceBetween: 0,
...
let testimonialsSlider = new Swiper('.testimonials__slider', {
	...
	speed: 800,
	pagination: {
		el: '.testimonials__slider_paging',
		clickable: true,
	},
	breakpoints: {
		320: {
